...
if ("true".equals(checkStatus) && bookingId != null) {
    ...
    // Determine if this is a student booking or visitor booking based on ID format
    if (bookingId.contains("-STU-")) {
        // Student booking
        StudentBookingDAO studentBookingDAO = new StudentBookingDAO();
        status = studentBookingDAO.getStatusByBookingID(bookingId);
    } else {
        // Visitor booking
        ReservationDAO reservationDAO = new ReservationDAO();
        status = reservationDAO.getStatusByBookingID(bookingId);
    }
    
    // Write JSON response directly
    out.print("{\"status\": " + (status == null ? "null" : "\"" + status + "\"") + ", \"bookingType\": \"" + 
              (bookingId.contains("-STU-") ? "student" : "visitor") + "\"}");
    out.flush();
                ...
                .then(function(stream) {
                    ...
                    codeReader.decodeFromVideoDevice(null, videoElement, function(result, err) {
                        if (result && !isProcessing) {
                            ...
                            fetch('checkin.jsp?checkStatus=true&bookingId=' + encodeURIComponent(result.text) + '&t=' + timestamp, {
                                method: 'GET',
                                headers: {
                            ...
                            // Update the fetch block inside startQRScanner function where it checks booking status
                            .then(text => {
                                try {
                                    ...
                                    if (data.status === "check-in") {
                                        ...
                                        // Send QR value to servlet based on booking type
                                        
                                        const servletURL = data.bookingType === "student" 
                                            ? '<%= request.getContextPath() %>/StudentCheckInServlet' 
                                            : '<%= request.getContextPath() %>/VisitorCheckInServlet';
